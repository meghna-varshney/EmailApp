            ...
            public void onClick(View view) {
               String s1 = email.getText().toString().trim();
                ...
                if (s1.equals("")||s2.equals("")||s3.equals(""))
                {
                    ...
                    Intent emailIntent = new Intent(Intent.ACTION_SEND, Uri.fromParts("mailto", s1, null));
                    emailIntent.putExtra(Intent.EXTRA_SUBJECT, s2);
                    emailIntent.putExtra(Intent.EXTRA_TEXT, s3);
                    if (emailIntent.resolveActivity(getPackageManager()) != null) {
                        startActivity(emailIntent);
                    }else{
                        Toast.makeText(MainActivity.this, " no activity found to handle intent", Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
